...
$(coremark-maxperf-Vibex):
	fusesoc \
		...
		--build \
		lowrisc:ibex:ibex_simple_system \
		`./util/ibex_config.py maxperf fusesoc_opts`
...
run-pmp-smoke-test: build-simple-system build-pmp-smoke-test-sw
	$(simple-system-Vibex) \
		--meminit=ram,$(pmp-smoke-test-bin)
	@echo ""
	cat ibex_simple_system.log
...
Vtb_cs_registers = build/lowrisc_ibex_tb_cs_registers_0/sim-verilator/Vtb_cs_registers
$(Vtb_cs_registers):
	@echo "$@ not found"
